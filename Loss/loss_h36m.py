...
def mpjpe_cal(predicted, target):
    ...
    Calculate MPJPE.
    Args:
        predicted (torch.Tensor): Predicted 3D joint coordinates.
        target (torch.Tensor): Ground truth 3D joint coordinates.
    Returns:
        float: Mean per-joint position error.
    """
    assert predicted.shape == target.shape, f"Predicted shape is {predicted.shape}, target shape is {target.shape}"
